...
Create and launch AI assistant/chatbot for your Bagisto website in minutes. It talks to your visitors, helps, explains, never misses a chat and can increase conversion rates! All while knowing your website, customized just for you. Your Bagisto website can now talk.

This plugin embeds the Asyntai chatbot on your Bagisto site and provides a simple admin interface to connect your site to Asyntai.
...
```bash
composer require asyntai/bagisto-chatbot
```
...
1. Copy the `bagisto` folder to `packages/Asyntai/Chatbot` in your Bagisto project

2. Add the following to your Bagisto's `composer.json` in the `autoload.psr-4` section:
   ```json
   "Asyntai\\Chatbot\\": "packages/Asyntai/Chatbot/src"
   ```
...
3. Register the service provider in `bootstrap/providers.php`:
   ...
   return [
       // ... other providers
       Asyntai\Chatbot\Providers\ChatbotServiceProvider::class,
   ];
   ```
...
1. Go to Admin Panel -> Asyntai Chatbot
...
- Bagisto 2.0 or higher
- PHP 8.1 or higher
- Laravel 10.x or higher
- Backend admin access for configuration
- If your Bagisto version is not compatible, you can still use Asyntai by getting a JavaScript snippet from [asyntai.com/dashboard](https://asyntai.com/dashboard)
